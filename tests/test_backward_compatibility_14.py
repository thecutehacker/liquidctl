...
def test_kraken2_backwards_modes_are_deprecated(caplog):
    ...
    for mode in modes:
        ...
        old = Kraken2(MockHidapiDevice(), "Mock X62", device_type=Kraken2.DEVICE_KRAKENX)
        new = Kraken2(MockHidapiDevice(), "Mock X62", device_type=Kraken2.DEVICE_KRAKENX)
        ...
        assert old.device.sent == new.device.sent, f"{mode} != {base_mode} + direction=backward"
...
def test_kraken3_backwards_modes_are_deprecated(caplog):
    modes = [
        ...
        "backwards-marquee-5",
        "backwards-marquee-6",
        "backwards-moving-alternating-3",
        "covering-backwards-marquee",
        "backwards-moving-alternating-4",
        "backwards-moving-alternating-5",
        "backwards-moving-alternating-6",
...
def test_smart_device_v1_backwards_modes_are_deprecated(caplog):
    ...
    for mode in modes:
        ...
        old = SmartDevice(
            MockHidapiDevice(), "Mock Smart Device", speed_channel_count=3, color_channel_count=1
        )
        new = SmartDevice(
            MockHidapiDevice(), "Mock Smart Device", speed_channel_count=3, color_channel_count=1
        )
...
def test_hue2_backwards_modes_are_deprecated(caplog):
    ...
    for mode in modes:
        ...
        old = SmartDevice2(
            MockHidapiDevice(), "Mock Smart Device V2", speed_channel_count=3, color_channel_count=2
        )
        new = SmartDevice2(
            MockHidapiDevice(), "Mock Smart Device V2", speed_channel_count=3, color_channel_count=2
        )

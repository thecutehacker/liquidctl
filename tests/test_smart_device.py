...
def mockSmartDevice():
    ...
    return SmartDevice(
        device, "mock NZXT Smart Device V1", speed_channel_count=3, color_channel_count=1
    )
...
def test_smart_device_constructor(mockSmartDevice):
    ...
    assert mockSmartDevice._color_channels == {
        "led": (0),
    }
...
def test_smart_device_not_totally_broken(mockSmartDevice):
    ...
    dev.set_color(channel="led", mode="breathing", colors=iter([[142, 24, 68]]), speed="fastest")

...
def make_pypi_long_description(doc_url):
    ...
    with open("README.md", "r", encoding="utf-8") as fh:
        continuation = (
            "For which devices are supported, installation instructions, "
            "a guide to the CLI and device specific details, check the "
            "complete [Documentation]({})."
        ).format(doc_url)
        long_description = fh.read().split("<!-- stop here for PyPI -->", 1)[0] + continuation
...
def make_extraversion(editable=False):
    ...
    if get_git_version() and os.path.isdir(".git"):
        ...
        describe = (
            subprocess.check_output(["git", "describe", "--always", "--dirty"]).strip().decode()
        )

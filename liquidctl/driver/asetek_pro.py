...
_COLOR_SPEEDS_VALUES = {
    "shift": [0x46, 0x28, 0x0F],  # slower  # normal  # faster
    "pulse": [0x50, 0x37, 0x1E],  # slower  # normal  # faster
    "blinking": [0x0F, 0x0A, 0x05],  # slower  # normal  # faster
}
...
class CorsairAsetekProDriver(_CommonAsetekDriver):
    ...
    def set_color(self, channel, mode, colors, speed="normal", **kwargs):
        ...
        if mode == "alert":
            ...
            temps = (30, 40, 50)
            self._post(
                [0x5F, temps[0], 0x00, temps[1], 0x00, temps[1], 0x00]
                + colors[0]
                + colors[1]
                + colors[2],
                read_length=6,
    ...
    def set_speed_profile(self, channel, profile, **kwargs):
        ...
        for temp, duty in adjusted:
            _LOGGER.info(
                "setting %s PWM point: (%i°C, %i%%), device interpolated", channel, temp, duty
            )

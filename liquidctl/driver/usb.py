...
class PyUsbDevice:
    ...
    def open(self, bInterfaceNumber=0):
        ...
        if sys.platform.startswith("linux") and self.usbdev.is_kernel_driver_active(
...
class HidapiDevice:
    ...
    def write(self, data):
        ...
        """
        _LOGGER.debug(
            "writting report 0x%02x with %d bytes: %r",
            data[0],
            len(data) - 1,
            LazyHexRepr(data, start=1),
        )
    ...
    def get_feature_report(self, report_id, length):
        ...
        _LOGGER.debug(
            "got feature report 0x%02x with %d bytes: %r",
            data[0],
            len(data) - 1,
            LazyHexRepr(data, start=1),
        )
    ...
    def send_feature_report(self, data):
        ...
        """
        _LOGGER.debug(
            "sending feature report 0x%02x with %d bytes: %r",
            data[0],
            len(data) - 1,
            LazyHexRepr(data, start=1),
        )
...
class HidapiBus(BaseBus):
    def find_devices(
        self, vendor=None, product=None, bus=None, address=None, usb_port=None, **kwargs
    ):
        """Find compatible HID devices."""
        handles = HidapiDevice.enumerate(hid, vendor, product)
        drivers = sorted(
            find_all_subclasses(UsbHidDriver), key=lambda x: (x.__module__, x.__name__)
        )
        _LOGGER.debug(
            "searching %s (%s)",
        ...
        for handle in handles:
            ...
            if usb_port and handle.port != usb_port:
                continue
            _LOGGER.debug("found HID device %04x:%04x", handle.vendor_id, handle.product_id)
            for drv in drivers:
...
class PyUsbBus(BaseBus):
    def find_devices(
        ...
        drivers = sorted(find_all_subclasses(UsbDriver), key=lambda x: (x.__module__, x.__name__))
        _LOGGER.debug(
            "searching %s (%s)",
            self.__class__.__name__,
            ", ".join(map(lambda x: x.__name__, drivers)),
        ...
        for handle in PyUsbDevice.enumerate(vendor, product):
            ...
            if usb_port and handle.port != usb_port:
                continue
            _LOGGER.debug("found USB device %04x:%04x", handle.vendor_id, handle.product_id)
            for drv in drivers:
                yield from drv.probe(handle, vendor=vendor, product=product, **kwargs)
